...
{
    // GPM[3:0] for LED
    rGPMCON = (rGPMCON & ~(0xff<<0))|(0x55<<0);    //GPM[3:0] for LED as output
    rGPMPUD = (rGPMPUD & ~(0xff<<0));                // Pull-Down/Up Disable
    //rGPFCON = (rGPFCON & ~(0x3<<30))|(0x1<<30);         
...
void Led_Display(int data)
{
    ...
    //
    rGPMDAT = (rGPMDAT & ~(0xf<<0)) | ((data & 0xf)<<0);
}
...
void Uart_Init(void)
{
    UINT32 DivSlot;
    float Div;
    rPCLK_GATE |= (1<<1);               // UART0
    rSCLK_GATE |= (1<<5);               // UART0~3	
